...
Handles long-running tasks like ML retraining and massive report generation
...
def train_ml_models(self):
    ...
    # Simulate a long running ML training process
    time.sleep(5)
    
    # Ideally we'd call the pipeline here:
    # from ml_models.demand_forecaster import main as train_demand
    # train_demand()
    
    logger.info("ML retraining complete.")
    return {"status": "success", "models": ["xgboost_demand", "lstm_forecast"]}

@app.task(bind=True, name="generate_executive_report")
def generate_executive_report(self):
    """Background task to generate the massive PDF executive report."""
